    ...
    <div className="grid gap-5 xl:grid-cols-2">
      ...
      <section className="surface rounded-lg p-5">
        ...
        <form className="mt-5 grid gap-4" onSubmit={handleSave}>
          {[['full_name', 'Full name', 'text'], ['email', 'Email', 'email'], ['password', 'New password', 'password']].map(([key, label, type]) => (
            <label key={key} className="text-sm font-semibold text-slate-700">
              {label}
              <input type={type} value={form[key]} onChange={(e) => setForm((p) => ({ ...p, [key]: e.target.value }))} placeholder={key === 'password' ? 'Leave blank to keep current' : ''} className="mt-2 h-10 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm outline-none focus:border-[#0f766e] focus:bg-white" />
            </label>
          ))}
      ...
      <section className="surface rounded-lg p-5">
        ...
        <p className="mt-1 text-sm text-slate-500">Your current session.</p>
        ...
        <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 p-3 flex items-center gap-2">
          ...
          <p className="text-sm font-semibold text-emerald-800">Full admin access</p>
        </div>
        <button className="mt-4 w-full rounded-lg border border-rose-200 px-4 py-2.5 text-sm font-bold text-rose-600 hover:bg-rose-50" onClick={logout}>
          Sign out
        </button>
      </section>
    </div>
